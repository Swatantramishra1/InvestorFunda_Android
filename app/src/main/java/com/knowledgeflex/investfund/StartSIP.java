...
public class StartSIP extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_start_sip);
        if (getSupportActionBar() != null) {
            getSupportActionBar().setDisplayHomeAsUpEnabled(true);
            getSupportActionBar().setDisplayShowHomeEnabled(true);
            getSupportActionBar().setBackgroundDrawable(new ColorDrawable(Color.parseColor("#ff660033")));
        }

        years = (Spinner)findViewById(R.id.sip_years);
        risk = (Spinner)findViewById(R.id.sipRiskType);
        amount = (EditText) findViewById(R.id.monthlySipAmount);
        proceed = (Button) findViewById(R.id.sipInvestNow);
                    ...
                    if (Integer.parseInt(amt) >= 500) {
                        if (!years.getSelectedItem().toString().equals("Select Horizon")) {
                            if (!risk.getSelectedItem().toString().equals("Select Risk")) {
                                ...
                                sipInvest(split[0], risk.getSelectedItem().toString(), amount.getText().toString());
                            } else {
    ...
    private void sipInvest(String yrs, String risk, String amt) {
        try{
             sumCost = Integer.parseInt(amt);
            ...
            JSONObject obj = new CalculatePortfolio().CalculatePortfolioAllocation(durationYear, sumCost, risks, "","SIP");
            ...
            Intent i = new Intent(StartSIP.this, Child_Result.class);
            i.putExtra("page","9");
            i.putExtras(mBundle);
            i.putExtra("calculate", "SIP");
